...
export const BasicDetailsEdit = () => {
    ...
    const [userData, setUserData] = useState({salutation: '', first_name: '', last_name: '', email: '' });
    const [loading, setLoading] = useState(false);
    const [helperText, setHelperText] = useState({salutation: '', first_name: '', last_name: '', email: '' });
    const [errorMessages, setErrorMessages] = useState({salutation: false, first_name: false, last_name: false, email: false });
    ...
    const onSubmit = async () => {
        ...
        if(userDoc.exists()) {
            try {
                // Save user details to Firestore in the 'Basic_Details' collection using UID as the document ID
                await updateDoc(doc(db, "Basic_Details", params.id), userData);
                navigate(`/profile/view/${params.id}/?section=${section}`);
            } catch (error) {
            ...
            try {
                await setDoc(doc(db, "Basic_Details", params.id), userData);
                navigate(`/profile/view/${params.id}/?section=${section}`);
            } catch (error) {
        ...
        <div className="detail-form">            
            <Grid container spacing={2}>
                <Grid xs={12} sm={12} md={4} lg={4}>
                    <div className="title">Edit <b>Profile</b></div>
